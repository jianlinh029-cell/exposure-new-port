...
public class PhotographItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay tooltipDisplay, Consumer<Component> components, TooltipFlag tooltipFlag) {
        ...
        if (generation != null) {
            if (generation > 0)
                components.accept(Component.translatable("item.exposure.photograph.generation." + generation)
        ...
        if (Config.Client.PHOTOGRAPH_SHOW_PHOTOGRAPHER_IN_TOOLTIP.get() && !photographerName.isEmpty()) {
            components.accept(Component.translatable("item.exposure.photograph.photographer_tooltip",
        ...
        if (Config.Client.RECIPE_TOOLTIPS_WITHOUT_JEI.get()) {
            ...
            if ((generation == null || generation < 2) && !jeiLoaded) {
                ClientGUI.addPhotographCopyingTooltip(stack, context, components, tooltipFlag);
            }
        }
        ...
        if (tooltipFlag.isAdvanced()) {
            ...
            components.accept(Component.literal(identifier).withStyle(ChatFormatting.DARK_GRAY));
        }
    }
