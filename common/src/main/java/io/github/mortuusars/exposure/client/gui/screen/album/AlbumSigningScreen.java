...
public class AlbumSigningScreen extends Screen {
    ...
    // [SignDebug] log the first background draw once (render runs every frame).
    private boolean signBgLogged = false;
    ...
    protected void init() {
        ...
        Exposure.LOGGER.info("[SignDebug] screen opened leftPos={} topPos={} imageW={} imageH={}",
    ...
    public void renderBackground(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Fix: on 1.21.1 the call was blit(TEXTURE, x, y, blitOffset=0, u=298, v=0, w, h, 512, 512).
        // The 1.21.11 signature drops blitOffset, so the panel (horizontal offset 298 in the atlas)
        // must be sampled at u=298,v=0 -- the port had mistakenly shifted to u=0,v=298 (empty region),
        // which made the whole signing background/content invisible while widgets still rendered.
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, AlbumGUI.TEXTURE, leftPos, topPos, 298f, 0f,
                imageWidth, imageHeight, 512, 512);
        if (!signBgLogged) {
            signBgLogged = true;
            Exposure.LOGGER.info("[SignDebug] renderBg called texture={} leftPos={} topPos={} u=298 v=0 w={} h={}",
                    AlbumGUI.TEXTURE, leftPos, topPos, imageWidth, imageHeight);
        }
    }
    ...
    public boolean keyPressed(KeyEvent keyEvent) {
        ...
        if (titleTextBox.isFocused()) {
            return titleTextBox.keyPressed(keyEvent);
        }
