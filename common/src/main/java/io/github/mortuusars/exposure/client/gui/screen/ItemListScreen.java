...
public class ItemListScreen extends Screen {
    ...
    protected void renderBg(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, leftPos, topPos, 0f, 0f, imageWidth, 17, 256, 256);
        for (int i = 0; i < rowsCount; i++) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, leftPos, topPos + 17 + (i * 18), 0f, 17f, imageWidth, 18, 256, 256);
        }
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, leftPos, topPos + 17 + (rowsCount * 18), 0f, 35f, imageWidth, 7, 256, 256);

        for (Slot slot : slots) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, leftPos + slot.x - 1, topPos + slot.y - 1, 176f, 0f, 18, 18, 256, 256);
        }
    }
    ...
    protected void renderTooltip(GuiGraphics guiGraphics, int x, int y) {
        if (hoveredSlot != null && hoveredSlot.hasItem()) {
            ...
            guiGraphics.renderTooltip(font, tooltipComponents, x, y,
    ...
    public boolean keyPressed(KeyEvent keyEvent) {
        if (Minecrft.options().keyInventory.matches(keyEvent)) {
            onClose();
            return true;
        }
    ...
    public boolean mouseClicked(MouseButtonEvent mouseButtonEvent, boolean bl) {
        ...
        if (!isHovering(0, 0, imageWidth, imageHeight, mouseButtonEvent.x(), mouseButtonEvent.y())) {
            onClose();
            return true;
        }
