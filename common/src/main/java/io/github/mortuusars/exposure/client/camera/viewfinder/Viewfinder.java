...
public class Viewfinder {
    ...
    public boolean keyPressed(int key, int scanCode, int action, int modifiers) {
        return (action == InputConstants.PRESS && keyBindings.keyPressed(new KeyEvent(key, 0, modifiers)))
              || (action == InputConstants.RELEASE && keyBindings.keyReleased(new KeyEvent(key, 0, modifiers)))
    ...
    public boolean mouseClicked(int button, int action) {
        ...
        MouseButtonEvent event = new MouseButtonEvent(0, 0, new MouseButtonInfo(button, action));
