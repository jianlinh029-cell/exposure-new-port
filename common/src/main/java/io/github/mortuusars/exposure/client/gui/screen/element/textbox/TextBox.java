...
public class TextBox extends AbstractWidget {
    ...
    public boolean keyPressed(KeyEvent keyEvent) {
        if (!isFocused())
            return false;
        boolean handled = handleKeyPressed(keyEvent);
        if (handled)
            clearDisplayCache();
        return handled;
    }

    protected boolean handleKeyPressed(KeyEvent keyEvent) {
        int keyCode = keyEvent.input();
        boolean controlDown = (keyEvent.modifiers() & 2) != 0;
        TextFieldHelper.CursorStep cursorStep = controlDown ? TextFieldHelper.CursorStep.WORD : TextFieldHelper.CursorStep.CHARACTER;
        ...
        return textFieldHelper.keyPressed(keyEvent);
    }
    ...
    public boolean charTyped(CharacterEvent characterEvent) {
        ...
        boolean typed = textFieldHelper.charTyped(characterEvent);
        if (typed)
            clearDisplayCache();
        return typed;
    }
    ...
    public boolean mouseClicked(MouseButtonEvent mouseButtonEvent, boolean bl) {
        ...
        if (isHovered && visible && isActive() && mouseButtonEvent.button() == 0) {
            ...
            if (index >= 0) {
                if (index == lastIndex && currentTime - lastClickTime < 250L) {
                    ...
                    textFieldHelper.setCursorPos(index, Minecrft.hasShiftDown());
                }
                clearDisplayCache();
            }
    ...
    public boolean mouseDragged(MouseButtonEvent mouseButtonEvent, double dragX, double dragY) {
        if (mouseButtonEvent.button() == 0) {
            DisplayCache displayCache = this.getDisplayCache();
            int index = displayCache.getIndexAtPosition(this.font, this.convertScreenToLocal(new Pos2i((int) mouseButtonEvent.x(), (int) mouseButtonEvent.y())));
    ...
    protected void changeLine(int yChange) {
        ...
        this.textFieldHelper.setCursorPos(line, Minecrft.hasShiftDown());
    }

    protected void keyHome(boolean controlDown) {
        if (controlDown) {
            this.textFieldHelper.setCursorToStart(Minecrft.hasShiftDown());
        } else {
            int cursorIndex = this.textFieldHelper.getCursorPos();
            int lineStartIndex = this.getDisplayCache().findLineStart(cursorIndex);
            this.textFieldHelper.setCursorPos(lineStartIndex, Minecrft.hasShiftDown());
        }
    }

    protected void keyEnd(boolean controlDown) {
        if (controlDown) {
            this.textFieldHelper.setCursorToEnd(Minecrft.hasShiftDown());
        } else {
            DisplayCache displayCache = this.getDisplayCache();
            int cursorIndex = this.textFieldHelper.getCursorPos();
            int lineEndIndex = displayCache.findLineEnd(cursorIndex);
            this.textFieldHelper.setCursorPos(lineEndIndex, Minecrft.hasShiftDown());
        }
    }
}
