...
public class ClientEvents {
    ...
    public static boolean renderItemFrameItem(ItemStack stack, boolean isGlowFrame, int rotation,
                                              PoseStack poseStack, RenderQuadSink sink, int packedLight) {
        if (!Config.Client.PHOTOGRAPH_RENDERS_IN_ITEM_FRAME.get()) return false;
        if (!(stack.getItem() instanceof PhotographItem photographItem)) return false;
        if (photographItem.getFrame(stack).identifier().isEmpty()) return false;

        poseStack.pushPose();
        poseStack.scale(2F, 2F, 2F);
        ItemFramePhotographRenderer.render(stack, poseStack, sink, packedLight, photographItem, rotation, isGlowFrame);
        poseStack.popPose();
